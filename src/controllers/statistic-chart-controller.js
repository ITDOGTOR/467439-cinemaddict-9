...
import StatisticChart from '../components/statistic-chart.js';
...
export default class StatisticChartController {
  constructor(container, filmsData) {
    ...
    this._genres = getGenresCount(this._filmsData);
    this._chart = null;

    this._init();
  }

  updateChart(updatedFilmsData) {
    ...
    if (!(updatedFilmsData.filter((updatedFilmData) => updatedFilmData.isWatched).length)) {
      return;
    }

    this._chart = this._createGenresChart(this._genres);
  }
